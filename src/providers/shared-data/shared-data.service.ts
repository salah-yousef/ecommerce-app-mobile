...
export class SharedDataService {
  ...
  onStart() {
    //banners
    // this.config.getWoo("products?category=400" + "&" + this.config.productsArguments).then((data: any) => {
    //   var images = new Array(); 
    //   data.forEach( (element) => {
    //     images.push(element.images[0].src);
    //     console.log(element.images[0].src);
    //   });
    //   this.banners = images;
    // });

    this.config.getWithUrl("https://ahlmadinah.com/wp-json/wc/v3/ahlmadinah/slider").then((data: any) => {
      console.log(data);
      var images = new Array();
      data.forEach( (element) => {
        var slide = new Array();
        let image = JSON.parse(element.params).backgroundImage;
        let description = element.description;
        image = image.substring(image.indexOf("/") + 1);
        slide.push('https://ahlmadinah.com/wp-content/uploads/'+image);
        slide.push(description);
        images.push(slide);
      });
      this.banners = images;
    });
    ...
    //getting tab 2
    this.config.getWoo("products?total_sales=true&order=desc&orderby=price" + "&" + this.config.productsArguments).then((data: any) => {
      this.tab2 = data
      this.applicationRef.tick();
    });
    //getting tab 3
    ...
    this.config.getWithUrl("https://ahlmadinah.com//wp-json/wp/v2/pages/695").then((data: any) => {
      let str = data.content.rendered.replace(/<p>(.*?)<\/p>/, '', '$1');
      str = str.replace(/<p>(.*?)<\/p>/, '', 'and$1');
      this.config.termServices = str;
      
    });
    this.config.getWithUrl("https://ahlmadinah.com//wp-json/wp/v2/pages/3").then((data: any) => {
      let str = data.content.rendered.replace(/<p>(.*?)<\/p>/, '', '$1');
      str = str.replace(/<p>(.*?)<\/p>/, '', 'and$1');
      this.config.privacyPolicy = str;
    });
    this.config.getWithUrl("https://ahlmadinah.com//wp-json/wp/v2/pages/763").then((data: any) => {
      this.config.refundPolicy = data.content.rendered;      
    });
    //getting all allCategories
    this.getAllCategories(1);
    this.getfilteredCategories(1);
  }
  ...
  getfilteredCategories(page) {
    this.config.getWoo("products/categories?include=118,119,120,121,122,123&" + this.config.productsArguments).then((data: any) => {
      let dat = data
    ...
    let count = 0;
      for (let value of this.cartProducts) {
        ...
        if (p.type != 'variable' && value.product_id == p.id) {
           count++; value.quantity = parseInt(value.quantity) + parseInt(quantity); 
          } else if(vId) {
            if (value.product_id == p.id && value.variation_id == vId.id) { 
              count++; value.quantity = parseInt(value.quantity) + parseInt(quantity); 
            }
          }
      }
  ...
  getChilderenPageById(id: number) {
    var pages = new Array(); 
    this.config.getDataHttp("https://ahlmadinah.com/wp-json/wp/v2/pages?_embed&parent=" + id).then((data: any) => {
         data.forEach( (element) => {
          pages.push(element);
         });
       });
  ...
  getPageById(id: number){
    return this.config.getDataHttp("https://ahlmadinah.com/wp-json/wp/v2/pages/" + id);
  }
  //=================================================================================================================================
