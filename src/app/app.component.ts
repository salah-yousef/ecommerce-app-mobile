import { Component, NgZone, Renderer2, ElementRef, ViewChild, OnInit } from '@angular/core';
...
export class AppComponent implements OnInit{
  @ViewChild('myFabButton', { read: ElementRef,static: true }) myFabButton: ElementRef; 
  @ViewChild('myFabButtonIcon', { read: ElementRef,static: true }) myFabButtonIcon: ElementRef; 
  myCategories = [];
  isMenuOpen = false;
  rootPage: any;
  appPages = [];
  ...
  menuOpened() {
    this.menuCtrl.isOpen('mainMenu').then(res => {
      this.isMenuOpen = res;
    });
    this.myFabButton.nativeElement.classList.remove("ballBack"); 
    this.myFabButton.nativeElement.classList.add("ball");
    this.myFabButtonIcon.nativeElement.setAttribute("name", "arrow-back-circle");
  ...
  menuClosed() {
    ...
    this.myFabButton.nativeElement.classList.remove("ball"); 
    this.myFabButton.nativeElement.classList.add("ballBack");
    this.myFabButtonIcon.nativeElement.setAttribute("name", "arrow-forward-circle");
    console.log(this.isMenuOpen);
  }
  onTouch(v) {
    this.myFabButton.nativeElement.style = "display:block";
    setTimeout(() => {
      this.menuCtrl.isOpen('mainMenu').then(res => {
        this.isMenuOpen = res;
        if (!this.isMenuOpen) {
          this.myFabButton.nativeElement.style = "display:none;";
        }
      });
    }, 5000);
  }
  ...
  getStatusBarColor() {
    ...
    rgb2 = headerColor.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+)/i).toString();
    ...
    let rgb = color;
    rgb = color.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+)/i).toString();
