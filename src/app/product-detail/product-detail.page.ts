import { Component, OnInit, ApplicationRef, ViewEncapsulation, ViewChild, ElementRef } from '@angular/core';
...
export class ProductDetailPage implements OnInit {
  ...
  pId: any;
  @ViewChild('firstSlider', { read: ElementRef,static: true }) firstSlider: ElementRef;
  ...
  constructor(
    ...
    this.product.description = this.product.description.replace(/h5/g, 'h6');
  ...
  swipe(i,image){
    this.firstSlider.nativeElement.length().then((data) => {
      for(let i = 0; i < data; i++){
        document.getElementsByClassName("image"+i)[0].setAttribute("style", "border: none");
      }
      image.setAttribute("style", "border: thin solid blue");
    });
    this.firstSlider.nativeElement.slideTo(i);
  }
  getProductData(id) {
    let p;
  ...
  getDescription(data) {
    if (JSON.stringify(data).match(/<h5[^>]*>(.*?)<\/h5>/g) !== null) {
      let str = JSON.stringify(data).match(/<h5[^>]*>(.*?)<\/h5>/g).toString();      
  ...
  fillAttributes = function (val, key, position) {
    ...
    if ((this.selectAttributes.length != this.getAttributesLength()) && (JSON.parse(JSON.stringify(this.getProductData(this.pId))) !== null)) {
  ...
  selectVariation() {
    ...
    if ((this.selectAttributes !== null) && (this.selectedVariation !== null))
