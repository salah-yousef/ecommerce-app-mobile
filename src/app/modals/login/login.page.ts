...
export class LoginPage implements OnInit {
  ...
  googleLogin() {
    ...
    this.googlePlus.login({})
      .then(res => {
        ...
        this.createAccount(res.accessToken, 'google');
      })
  ...
  createAccount(info, type) {
    if (type === "fb") {
        ...
        url1 = 'https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=' + info;

        this.http.get(url1).subscribe((data1: any) => {
            var url2 = '';
            url2 = '/api/appusers/google_connect/?insecure=cool&email=' + data1.email + '&name=' + data1.name + '&given_name=' + data1.given_name + '&family_name=' + data1.family_name;
            this.http.get(this.config.url + url2).subscribe((data2: any) => {
              this.loading.hide();  
              console.log(data2);
              this.getUserData(data2, "google");
            }, error => {
              this.loading.hide();
              this.shared.showAlert("error " + JSON.stringify(error));
            });
